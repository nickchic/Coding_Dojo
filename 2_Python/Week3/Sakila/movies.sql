...
-- feature = behind the scenes", joined by actor_id = 15? Your query should return the 
-- film title, description, release year, rating, and special feature. Hint: You may use 
-- LIKE function in getting the 'behind the scenes' part.

SELECT film.title, film.description, film.release_year, film.rating, film.special_features
FROM film
LEFT JOIN film_actor
ON film.film_id = film_actor.film_id
WHERE film_actor.actor_id = 15 AND film.special_features LIKE '%behind the scenes%' AND rating = 'G';
...
-- 7. What query would you run to get all drama films with a rental rate of 2.99? Your 
-- query should return film title, description, release year, rating, special features, and genre (category).
